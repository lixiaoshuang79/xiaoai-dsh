...
**确定性短路**：空调调温（「空调 + 数字温度」）先于所有路由判断——红外空调
无状态回读，模型工具链会反复搜索/多轮操作，桥侧正则直连 HA 一次设值完成。
...
- **提醒队列 3 工具**：reminder_set / reminder_list / reminder_cancel——本地
  ...
  （官方小爱已全禁，闹钟/提醒由桥侧接管）。
- **空调温度确定性短路**：`_ac_temp_shortcut`——「空调 + 数字温度」正则直连
  HA number 实体 set_value（先于所有路由；带「开」字先按开机键 + 0.5s 间隔；
  范围 16-30）。
