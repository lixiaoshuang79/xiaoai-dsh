...
`_ac_shortcut` 直连 HA number 实体 set_value（带「开」字先按开机键
+ 0.5s 间隔；范围 16-30），先于所有路由判断一次完成。提示词的设备规则
同步立铁律：只用 number 实体 set_value，禁温度±按钮，禁反复查询。后来
同一模式扩展到塔扇/扫地机/摄像头（见 architecture.md），新增无回读设备
指令时照此模式做确定性短路。
