...
impl AppClient {
    ...
    async fn init(&mut self, ws_stream: WsStream) {
        ...
        // 可选认证：配置了 XIAOAI_WS_SECRET 时，把 {"auth":"<secret>"} 作为首条消息发出。
        // 服务端（migpt server.rs）在握手后 5 秒内等待该消息，校验通过才接入；
        // 未配置该环境变量时保持原行为（不发送认证消息，兼容未启用认证的服务端）。
        if let Ok(secret) = std::env::var("XIAOAI_WS_SECRET") {
            if !secret.is_empty() {
                let auth_msg = serde_json::json!({ "auth": secret }).to_string();
                println!("🔑 发送认证消息");
                if let Err(e) = MessageManager::instance()
