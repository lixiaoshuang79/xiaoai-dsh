...
impl MessageManager {
    ...
    pub async fn process_messages(&self) -> Result<(), AppError> {
        ...
        // 每连接独立限流器：文本 ≤1MiB、二进制 ≤512KiB，record 帧令牌桶限速；
        // 超限计次达到上限返回 Err，由调用方断开连接。
        ...
        loop {
            ...
            match reader.as_mut() {
                None => break,
                Some(WsReader::Client(reader)) => next_msg = reader.next().await,
                Some(WsReader::Server(reader)) => next_msg = reader.next().await,
            }
            drop(reader);
            match next_msg {
                None => break,
                ...
                Some(Ok(msg)) => {
                    match msg {
                        Message::Text(text) => {
                            limiter.check_text(text.len())?;
                            let _ = self.on_text(text.to_string()).await;
                        }
                        Message::Binary(bytes) => {
                            let stream_bytes: Vec<u8> = bytes.into();
                            let len = stream_bytes.len();
                            limiter.check_binary(len)?;
                            let _ = self.on_bytes(stream_bytes, &mut limiter).await;
                        }
                        _ => {}
                    };
                }
    ...
    async fn on_bytes(
        ...
        // record 音频帧额外走令牌桶，防灌流刷爆下游（JS on_input_data）
        if data.tag == "record" {
            limiter.check_record_frame(data.bytes.len())?;
        }
